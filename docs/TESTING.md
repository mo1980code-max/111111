...
```
python3 -m unittest discover -s tests -v      # 95 checks, ~11 s, no dependencies
```
...
| `test_kotlin_structure.py` | 13 | `kotlinc` hygiene: package == directory, balanced brackets, imports resolve, are unique and are all used, every project symbol used in a file is imported, Hilt annotations paired, no TODO / placeholder / empty function bodies |
...
Audits that always pass are worthless, so the two heaviest ones were checked against injected
faults and both failed as intended before the file was restored:
