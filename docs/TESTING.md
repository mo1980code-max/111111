...
```
python3 -m unittest discover -s tests -v      # 108 checks, ~13 s, no dependencies
```
...
| `test_product_contracts.py` | 54 | The specification: permission budget, no network / SDK / exact alarms / WorkManager / service, overlay dismiss-and-open-nothing, window context, quiet hours, boot + time-change restore, no postponed chain, widget rules, RTL, splash, accessibility, Room safety |
